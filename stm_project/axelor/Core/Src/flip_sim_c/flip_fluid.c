...
// Round 9 (axelor only): every pointer below was a malloc/calloc target on the host; this
// board has no heap (research.md Decision 19), so each becomes a static array sized to this
// project's one fixed tank configuration (FF_GRID_CELLS/FF_MAX_PARTICLES from flip_fluid.h;
// FF_HASH_CELLS below).
static float g_particleVel[FF_MAX_PARTICLES * 2]; // size 2 * maxParticles
static int g_gridInitialized = 0;

// --- Grid-based state (PIC/FLIP fields) ---
static int g_fNumX = 0, g_fNumY = 0, g_fNumCells = 0;
static float g_u[FF_GRID_CELLS], g_v[FF_GRID_CELLS];         // grid velocities
static float g_du[FF_GRID_CELLS], g_dv[FF_GRID_CELLS];       // weights accumulation
static float g_prevU[FF_GRID_CELLS], g_prevV[FF_GRID_CELLS]; // previous velocities
static float g_p[FF_GRID_CELLS];                             // pressure
static float g_particleDensity[FF_GRID_CELLS];               // per-cell particle density
static int   g_cellType[FF_GRID_CELLS];                      // 0 fluid, 1 air, 2 solid
static float g_particleRestDensity = 0.0f;          // computed once from fluid cells

// --- Particle spatial hash grid ---
#define FF_HASH_CELLS 625 // pNumX(25)*pNumY(25), from this tank's fixed r/tankWidth/tankHeight
static float g_pInvSpacing = 0.0f;
static int g_pNumX = 0, g_pNumY = 0, g_pNumCells = 0;
static int g_numCellParticles[FF_HASH_CELLS];
static int g_firstCellParticle[FF_HASH_CELLS + 1]; // length pNumCells+1
static int g_cellParticleIds[FF_MAX_PARTICLES];    // length maxParticles
...
static void ensure_grid_alloc(const FlipFluid *f) {
    ...
    if (wantX == g_fNumX && wantY == g_fNumY && g_gridInitialized) return;
    memset(g_u, 0, sizeof(g_u));
    memset(g_v, 0, sizeof(g_v));
    memset(g_du, 0, sizeof(g_du));
    memset(g_dv, 0, sizeof(g_dv));
    memset(g_prevU, 0, sizeof(g_prevU));
    memset(g_prevV, 0, sizeof(g_prevV));
    memset(g_p, 0, sizeof(g_p));
    memset(g_particleDensity, 0, sizeof(g_particleDensity));
    memset(g_cellType, 0, sizeof(g_cellType));
