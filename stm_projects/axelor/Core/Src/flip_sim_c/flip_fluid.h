...
// Round 9 (axelor only): static-allocation maxima replacing the original malloc/calloc
// sizing (research.md Decision 19) — this board has no heap, and this project has exactly
// one fixed tank configuration, so these are the literal results of setupScene()'s own
// formulas for that configuration, not arbitrary margins.
#define FF_GRID_CELLS (PAD_FNUM_X * PAD_FNUM_Y) // 17*17=289, solver's padded grid
#define FF_MAX_PARTICLES 323                    // numX(17)*numY(19) hex-pack count
