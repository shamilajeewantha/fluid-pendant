...
// Round 9 (axelor only): static storage replacing the original malloc targets — this board
// has no heap, and this project has exactly one Scene/FlipFluid instance for its lifetime
// (research.md Decision 19).
static FlipFluid g_flipFluidInstance;
static float g_particlePosStorage[FF_MAX_PARTICLES * 2];
static float g_sStorage[FF_GRID_CELLS];
static float g_cellColorStorage[GRID_X * GRID_Y];
...
void setupScene(Scene *scene) {
    ...
    // Allocate fluid
    FlipFluid *f = &g_flipFluidInstance;
    ...
    f->particlePos = g_particlePosStorage;
    f->s = g_sStorage;
    f->cellColor = g_cellColorStorage; // 15x16 grid for cellColor
