...
`flip_fluid.h`. A binary flag flips fully the instant a jittering particle crosses one exact cell
boundary; thresholding the smoother density value, with a gap between the on/off thresholds,
prevents that sub-pixel jitter from blinking an LED every frame at rest.
