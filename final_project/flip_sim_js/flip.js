...
export class FlipFluid {
  constructor(density, width, height, spacing, particleRadius, maxParticles) {
    ...
    this.cellColor = new Float32Array(this.fNumCells);
  ...
  // Hysteresis threshold on particleDensity (already computed every tick for
  // solveIncompressibility's drift compensation) instead of the binary cellType flag —
  // research.md Decision 12. cellType flips fully the instant a jittering particle's
  // position crosses one exact cell boundary; particleDensity changes smoothly with
  // position, and the dead zone between the two thresholds absorbs that jitter so an
  // LED only changes when there's a real, sustained change in fluid coverage.
  updateCellColors() {
    for (var i = 0; i < this.fNumCells; i++) {
      var normalized =
