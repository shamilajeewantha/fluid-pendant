...
void setup() {
  ...
  Serial.println("[Before Measure Mode : STATUS]");
  for (int i = 0; i < 5; i++) {
  ...
  // ---- Step 1b: Read axis data (still before measure) ----
  Serial.println("[Before Measure Mode : VALUES]");
  for (int i = 0; i < 5; i++) {
    uint8_t x = readRegister(REG_XDATA);
    uint8_t y = readRegister(REG_YDATA);
    uint8_t z = readRegister(REG_ZDATA);
    uint8_t s = readRegister(REG_STATUS);

    Serial.printf("Cycle %d → X=%3d  Y=%3d  Z=%3d  STATUS=0x%02X\n", i, x, y, z, s);
    delay(500);
  }
  ...
  // --- Step 3: Read STATUS a few times after enabling measure mode ---
  Serial.println("[After Measure Mode : STATUS]");
  for (int i = 0; i < 5; i++) {
    uint8_t s = readRegister(REG_STATUS);
    printStatus(s);
    delay(500);
  ...
  Serial.println("[After Measure Mode : VALUES]");
}
