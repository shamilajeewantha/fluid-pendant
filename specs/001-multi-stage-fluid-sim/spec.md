...
- **(Revised 2026-06-21, research.md Decision 13)** Each LED cell in the Display Frame is treated as a continuous brightness/intensity value (derived from local particle density, clamped 0-1), not a binary on/off flag — this was the original assumption (binary occupancy, matching the early prototype's convention) but was revised once the simulator was running and a more realistic, equally lightweight rendering was found and validated against other real fluid-pendant projects. Per-LED brightness is achievable on the real hardware via cheap bit-angle-modulation PWM within the existing charlieplex scan, so this does not raise the hardware's computational budget.
